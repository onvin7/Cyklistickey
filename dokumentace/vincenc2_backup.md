...
1. [ÚVOD](#úvod)		
2. [ANALÝZA OBDOBNÝCH WEBOVÝCH STRÁNEK](#1-analýza-obdobných-webových-stránek)
   1. [MTBS.CZ](#11-mtbscz)
      1. [Kladné stránky](#111-kladné-stránky)
      2. [Záporné stránky](#112-záporné-stránky)
   2. [ROADCYCLING.CZ](#12-roadcyclingcz)
      1. [Kladné stránky](#121-kladné-stránky)
      2. [Záporné stránky](#122-záporné-stránky)
   3. [MTBIKER.SK](#13-mtbikersk)
      1. [Kladné stránky](#131-kladné-stránky)
      2. [Záporné stránky](#132-záporné-stránky)
3. [NÁVRH PROJEKTU](#2-návrh-projektu)
   1. [CÍLOVÉ SKUPINY](#21-cílové-skupiny)
   2. [ADMINISTRACE WEBU](#22-administrace-webu)
   3. [DATABÁZE](#23-databáze)
   4. [DESIGN A RESPONZIVITA](#24-design-a-responzivita)
4. [POPIS PROJEKTU](#3-popis-projektu)
   1. [FRONTEND](#31-frontend)
   2. [BACKEND](#32-backend)
5. [ZÁVĚR](#závěr)
6. [SEZNAM PŘÍSTUPOVÝCH ÚDAJŮ](#seznam-přístupových-údajů)	
7. [SEZNAM POUŽITÉ LITERATURY A ZDROJŮ OBRÁZKŮ](#seznam-použité-literatury-a-zdrojů-obrázků)
8. [SEZNAM OBRÁZKŮ](#seznam-obrázků)
9. [PŘÍLOHY](#přílohy)
...
> [VLOŽIT OBRÁZEK 6: Dashboard administračního rozhraní zobrazující přehled nejdůležitějších statistik a funkcí]
...
* **Propagace článků** - možnost nastavit zvýrazněné články na hlavní stránce s možností plánování
...
**Správa článků:**
* TinyMCE WYSIWYG editor pro pohodlnou tvorbu a úpravu článků
...
> [VLOŽIT OBRÁZEK 7: TinyMCE WYSIWYG editor pro tvorbu článků s nástroji pro formátování textu a vkládání médií]
...
* Správa odkazů na sociální sítě uživatelů

**Systém propagace článků:**
* Možnost zvýraznit vybrané články na hlavní stránce
* Nastavení priorit propagovaných článků
* Plánování začátku a konce propagace
* Historie propagací a přehled budoucích propagací

**Přístupová práva:**
* Konfigurace přístupů k různým částem administrace
* Omezení funkcí podle uživatelské role (redaktor, editor, administrátor)
* Dynamické řízení přístupu ke stránkám pomocí tabulky admin_access

**Statistiky:**
* Sledování počtu zobrazení jednotlivých článků
* Přehled čtenosti podle kategorií
* Statistiky autorů a jejich aktivity
* Grafické znázornění statistik
* Detailní statistiky konkrétních článků, kategorií a autorů

**Technické specifikace administrace:**
...
> [VLOŽIT OBRÁZEK 33: Sekce se souvisejícími články na konci stránky článku]

Audio přehrávač je plně integrován do stránky a umožňuje uživatelům poslouchat článek, zatímco se věnují jiným aktivitám. Přehrávač využívá standardní HTML5 audio element, který poskytuje nativní ovládací prvky podle prohlížeče uživatele. Implementace v souboru **app/Views/Web/articles/article.php**:

```php
<!-- Kód HTML pro audio přehrávač -->
<div class="prehravac">
    <audio controls>
...
Použití standardního HTML5 elementu `<audio>` zajišťuje kompatibilitu se všemi moderními prohlížeči a nabízí uživatelům intuitivní rozhraní podle zvyklostí jejich prohlížeče. Tento přístup také zajišťuje lepší přístupnost a kompatibilitu s mobilními zařízeními.
...
**Mobilní zařízení (< 576px):**
* Jednosloupová struktura s vertikálním řazením všech elementů
* Kompaktní "hamburger" menu místo horizontální navigace
* Menší velikost fontů a obrázků pro úsporu místa
* Zjednodušené karty článků s menšími náhledovými obrázky
* Optimalizované formuláře s většími dotykovými plochami
* Skryté některé méně důležité elementy pro zjednodušení rozhraní

> [VLOŽIT OBRÁZEK 36: Detail zobrazení webu na mobilním telefonu s jednosloupovou strukturou]

**Tablety (576px - 991px):**
* Dvousloupová struktura pro lepší využití prostoru
* Kombinace horizontálního menu pro hlavní kategorie a hamburger menu pro podkategorie
* Větší karty článků s většími náhledovými obrázky
* Zobrazení více doplňkových informací v porovnání s mobilním zobrazením

> [VLOŽIT OBRÁZEK 37: Detail zobrazení webu na tabletu s dvousloupovou strukturou]

**Desktopy (≥ 992px):**
* Třísloupcová struktura využívající prostor velkých obrazovek
* Plné horizontální menu se všemi kategoriemi
* Maximální množství zobrazených informací
* Rozšířené doplňkové funkce jako boční panel s populárními články, kalendář událostí atp.

> [VLOŽIT OBRÁZEK 38: Detail zobrazení webu na desktopovém počítači s třísloupcovou strukturou]
...
> [VLOŽIT OBRÁZEK 39: Screenshot z Chrome DevTools ukazující testování responzivity na různých zařízeních]
...
> [VLOŽIT OBRÁZEK 40: Detail audio přehrávače s ovládacími prvky]
...
> [VLOŽIT OBRÁZEK 17: Ukázka administrace článků s možnostmi pro vytváření, editaci a mazání obsahu]
